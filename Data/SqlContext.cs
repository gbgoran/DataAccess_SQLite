    ...
    public static class SqliteContext
    {
        ...
        public static async Task CreateCommentAsync(Comment comment)
        {
            ...
            using (var db = new SqliteConnection(_dbpath))
            {
                ...
                cmd.Parameters.AddWithValue("@IssueId", comment.FkIssueId);
        ...
        public static async Task<IEnumerable<Customer>> GetCustomers()
        {
            ...
            using (var db = new SqliteConnection(_dbpath))
            {
                ...
                if (result.HasRows)
                {
                    while (result.Read())
                    {
                        customers.Add(new Customer(result.GetInt32(0), result.GetString(1), result.GetString(2), result.GetInt32(3)));
                    }
                }
        ...
        public static async Task<Customer> GetCustomerById(long id)
        {
            ...
            using (var db = new SqliteConnection(_dbpath))
            {
                ...
                if (result.HasRows)
                {
                    while (result.Read())
                    {
                        customer = new Customer(result.GetInt32(0), result.GetString(1), result.GetString(2), result.GetInt32(3));
                    }
                }
        ...
        public static async Task<IEnumerable<Issue>> GetIssues()
        {
            ...
            using (var db = new SqliteConnection(_dbpath))
            {
                ...
                if (result.HasRows)
                {
                    ...
                    while (result.Read())
                    {
                        var issue = new Issue(result.GetInt32(0), result.GetInt32(1), result.GetString(2), result.GetString(3), result.GetString(4));
